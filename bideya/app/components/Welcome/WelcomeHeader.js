...
export default function WelcomeHeader() {
  ...
  const navItems = [
    ...
    { label: "Booking", href: "/boock" },
    { label: "Podcast", href: "/PodcastPage" }, // Updated href
    { label: "Reels", href: "/Reelpage" }, // Updated href
        ...
        <ul className="md:flex space-x-12 text-blue-900 text-xl font-semibold">
          {navItems.map((item) => (
            <li key={item.label}>
              <Link
                href={item.href}
                className="hover:text-blue-700" // Removed the conditional border
              >
                {item.label}
              </Link>
            </li>
